...
| [Observability](../../observability/best-practices.md) | The ability to measure a system's internal state and performance based on the outputs it generates, such as logs, metrics, and traces. | -System Metrics: CPU usage, memory usage, disk I/O, network I/O, and other resource utilization metrics.<BR/>- Application Metrics: Response times, request rates, error rates, and throughput.<BR/>- Custom Metrics: Application-specific metrics, such as user sign-ups, or specific business logic indicators. |
...
Conduct a design review for:

- Major new components or services being added to the system
- Architectural decisions that affect multiple teams or have long-term implications
- Significant changes to existing architecture (new patterns, technology migrations)
- Any decision where alternatives were considered and trade-offs matter

Design reviews should be documented and stored alongside the code. Include alternatives considered and the rationale for the chosen approach. Stories and/or PRs should link to the design document.

### Trade Study Flow

A trade study is a structured way to choose between competing technical options:

1. **Requirements** — Define the criteria for success (functional requirements, NFRs, constraints)
2. **Criteria** — Weight the criteria by importance (e.g., performance = 30%, cost = 20%, maintainability = 50%)
3. **Options** — Enumerate the candidate solutions
4. **Compare** — Evaluate each option against each criterion; use a scoring matrix
5. **Decide** — Select the option with the best overall score; document the rationale and any dissenting views

### Decision Log

Capture architecture decisions as Architecture Decision Records (ADRs). Each ADR should include:

- **Context** — The situation or problem that drove the decision
- **Decision** — The change being proposed or made
- **Status** — Proposed / Accepted / Deprecated / Superseded
- **Consequences** — The resulting context after the decision is applied

> Full design review templates and ADR examples are available in the project archive.
